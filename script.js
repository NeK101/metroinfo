...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 로컬 역정보 로드 → 완료 후만 이벤트 등록
  let stations = [];
  fetch('data/stations.json')
    .then(r => {
      console.log('stations.json status:', r.status);
      return r.json();
    })
    .then(data => {
      ...
      console.log('Loaded stations count:', stations.length);
  ...
  async function getRealtimeArrivalAll(stationName) {
    const url = `http://swopenapi.seoul.go.kr/api/subway/${ALL_ARRIVAL}/xml/realtimeStationArrival/ALL`;
  ...
  async function getRealtimePosition(lineNo) {
    const url = `http://swopenapi.seoul.go.kr/api/subway/${POSITION_KEY}/xml/realtimePosition/0/100/${lineNo}`;
    const res = await fetch(url), xml = await res.text();
    const doc = new DOMParser().parseFromString(xml,'application/xml');
  ...
  async function getTimetableById(stationId, updnLine, weekTag) {
    const url = `http://openapi.seoul.go.kr:8088/${TIMETABLE_KEY}/xml/SearchSTNTimeTableByIDService/1/100/${stationId}/${updnLine}/${weekTag}`;
    const res = await fetch(url), xml = await res.text();
    const doc = new DOMParser().parseFromString(xml,'application/xml');
    return Array.from(doc.querySelectorAll('row')).map(r => ({
  ...
  async function getFirstLastByLine(stationId, lineNo, weekTag, updnLine) {
    ...
    const url = `http://openapi.seoul.go.kr:8088/${FIRSTLAST_KEY}/xml/SearchFirstAndLastTrainbyLineServiceNew/1/5/${encodeURIComponent(lineName)}/${weekTag}/${updnLine}/${stationId}`;
    const res = await fetch(url), xml = await res.text();
    const doc = new DOMParser().parseFromString(xml,'application/xml');
